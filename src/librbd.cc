...
namespace librbd {
  ...
  struct ImageCtx {
    ...
    map<string, SnapInfo> snaps_by_name;
    uint64_t snap_id;
    ...
    snap_t get_snap_id(string snap_name) const
    {
    ...
    int get_snap_name(snapid_t snap_id, string *snap_name) const
    {
    ...
    int get_snap_size(string snap_name, uint64_t *size) const
    {
      map<string, SnapInfo>::const_iterator it = snaps_by_name.find(snap_name);
      if (it != snaps_by_name.end()) {
	*size = it->second.size;
...
struct CopyupProgressContext {
  CopyupProgressContext(ImageCtx *ictx, ProgressContext &p) :
		        clone_ictx(ictx), prog_ctx(p)
  {
  }
  ImageCtx *clone_ictx;
  ProgressContext &prog_ctx;
...
int copyup_block(ImageCtx *ictx, uint64_t offset, size_t len, const char *buf)
{
  ...
  string oid = get_block_oid(ictx->object_prefix, 
    get_block_num(ictx->order, offset), ictx->old_format);
  ictx->lock.Unlock();
...
int flatten(ImageCtx *ictx, ProgressContext &prog_ctx)
{
  ...
  while (ofs < ictx->parent_md.overlap) {
    ...
    size_t readsize = min(ictx->parent_md.overlap - ofs + 1, cblksize);
