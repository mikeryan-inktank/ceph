...
void PG::sub_op_scrub_map(OpRequestRef op)
{
  ...
  if (scrub_waiting_on == 0) { // chunky scrub
    if (scrub_is_chunky) {
      ...
      case SCRUB_NEW_CHUNK:
        ...
        // yuck, switch nastiness: locals to this state have to live inside a block
        {
          ...
          while (!boundary_found) {
            // FIXME deal with this returning < 0: SCRUB_INTERNAL_ERROR state?
            vector<hobject_t> objects;
            osd->store->collection_list_partial(coll, start, 5, 5, 0, &objects, &scrub_end);
