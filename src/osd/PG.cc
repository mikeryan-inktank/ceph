...
void PG::sub_op_scrub_map(OpRequestRef op)
{
  ...
  if (scrubber.waiting_on == 0) { // chunky scrub
  ...
  while (!done) {
    ...
    switch (scrubber.state) {
      ...
      case PG::Scrubber::NEW_CHUNK:
        ...
        // yuck, switch nastiness: locals to this state have to live inside a block
        {
          ...
          while (!boundary_found) {
            // FIXME deal with this returning < 0: PG::Scrubber::INTERNAL_ERROR state?
            vector<hobject_t> objects;
            osd->store->collection_list_partial(coll, start, 5, 5, 0, &objects, &scrubber.end);
...
void PG::scrub_clear_state()
{
  ...
  scrubber.finalizing = false;
  scrubber.block_writes = false;
  scrubber.active = false;
  scrubber.waiting_on = 0;
  scrubber.waiting_on_whom.clear();
  if (scrubber.active_rep_scrub) {
    scrubber.active_rep_scrub->put();
    scrubber.active_rep_scrub = NULL;
  }
  scrubber.received_maps.clear();
